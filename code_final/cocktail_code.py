...
def create_mix_profile(reconstructed_profile, ingredient_profiles, ingredient_names, k, total_volume=200.0):
    ...
    # Nur Top k behalten
    if k < len(w):
        sorted_idx = np.argsort(-w)  # absteigend
...
def questionnaire_and_cocktail_generator(data):
    ...
    # 4.4) Zutaten nach alc_pref filtern
    ...
    if alc_pref == "a":
        ...
        ]  # Alkoholisch und nicht-alkoholisch
    else:
        filtered_items = [
            (name, info["taste"])
            for name, info in all_ingredients.items()
    ...
    # 4.5) Autoencoder => Rekonstruktion
